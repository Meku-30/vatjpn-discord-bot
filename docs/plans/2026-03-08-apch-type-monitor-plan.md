...
ssh nas "cd <NAS_CONTAINER_STATION>/data/application/phase6-bots && HOME=/tmp DOCKER_CONFIG=/tmp/.docker <NAS_CONTAINER_STATION>/bin/docker compose restart bot1"
```

**Step 4: ログ確認**

```bash
ssh nas "cd <NAS_CONTAINER_STATION>/data/application/phase6-bots && HOME=/tmp DOCKER_CONFIG=/tmp/.docker <NAS_CONTAINER_STATION>/bin/docker compose logs --tail=30 bot1"
```
